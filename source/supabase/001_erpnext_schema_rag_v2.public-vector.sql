...
-- Variant for projects whose vector extension is installed in `public`.
...
begin
  ...
  elsif installed_schema <> 'public' then
    raise exception 'vector extension is installed in schema %, not public; use the matching schema-specific migration', installed_schema;
  end if;
end;
$$;
...
begin
  ...
  loop
    ...
    item_embedding := (item->'embedding')::text::public.vector(768);
...
revoke all on function public.match_erpnext_schema_v2(public.vector, text, integer) from public, anon, authenticated;
...
grant execute on function public.match_erpnext_schema_v2(public.vector, text, integer) to service_role;
